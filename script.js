...
document.querySelector('#case-grid').innerHTML=cases.map((c,i)=>`<article class="case-card reveal"><div class="case-art"><div class="mock-window"></div></div><div class="case-content"><span class="case-kicker">0${i+1} — CASE STUDY</span><h3>${c[0]}</h3><p>${c[1]}</p><div class="case-footer"><span class="case-tech">${c[2]}</span><a href="${c[3]}" target="_blank" rel="noopener" aria-label="Visit ${c[0]}">${icon('arrow-up-right')}</a></div></div></article>`).join('');
